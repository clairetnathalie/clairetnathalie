...
class Carabiner {
	 ...
	 * @return   Void
	 */
	public function display($flag = 'both', $group_filter = NULL, $async = FALSE) {
		
		switch ($flag) {
			
			case 'JS' :
			case 'js' :
				$this->_display_js ( NULL, $async );
				break;
	 ...
	 * @return   Void
	 */
	public function display_src($flag = 'both', $group_filter = NULL) {
		
		switch ($flag) {
			
			case 'JS' :
			case 'js' :
				$this->_display_js_src ();
				break;
			
			case 'CSS' :
			case 'css' :
				$this->_display_css_src ();
				break;
			...
			default :
				if (isset ( $this->js [$flag] ) && ($group_filter == NULL || $group_filter == 'js'))
					$this->_display_js_src ( $flag );
				
				if (isset ( $this->css [$flag] ) && ($group_filter == NULL || $group_filter == 'css'))
					$this->_display_css_src ( $flag );
				break;
		}
	}
	 ...
	 */
	public function display_string($flag = 'both', $group_filter = NULL, $async = FALSE) {
		...
		$this->display ( $flag, $group_filter, $async );
	 ...
	 * @return   String of HTML references
	 */
	public function display_src_string($flag = 'both', $group_filter = NULL) {
		ob_start (); // note: according to the manual, nesting ob calls is okay
		// so this shouldn't cause any problems even if you're using ob already
		

		$this->display_src ( $flag, $group_filter );
	 ...
	 * @return   Void
	 */
	private function _display_js($group = 'main', $async = FALSE) {
		...
		if ($this->dev) {
			...
			foreach ( $this->js [$group] as $ref ) :
				...
				echo $this->_tag ( 'js', $ref ['dev'], NULL, NULL, $async );
		...
		// if we're combining files and minifying them
			...
			foreach ( $this->js [$group] as $ref ) :
				...
				if (! $ref ['combine']) :
					echo (isset ( $ref ['prod'] )) ? $this->_tag ( 'js', $ref ['prod'], NULL, NULL, $async ) : $this->_tag ( 'js', $ref ['dev'], NULL, NULL, $async );
				 elseif (! $ref ['minify']) :
			...
			echo $this->_tag ( 'js', $filename, TRUE, NULL, $async );
		...
		// if we're minifying. but not combining
			...
			foreach ( $this->js [$group] as $ref ) :
				...
				echo $this->_tag ( 'js', $f, TRUE, NULL, $async );
		...
		// -- this will just display the production version if there is one, dev if there isn't.
			...
			foreach ( $this->js [$group] as $ref ) :
				...
				echo $this->_tag ( 'js', $f, NULL, NULL, $async );
	...
	/** 
	 * Display HTML SRC references to the js assets
	 * @access	private
	 * @param	String of the asset group name
	 * @return   Void
	 */
	private function _display_js_src($group = 'main') {
		...
		if ($this->dev) {
			...
			foreach ( $this->js [$group] as $ref ) :
				...
				echo (($count_0 > 0) ? ',' : '') . $this->_tag_src ( 'js', $ref ['dev'] );
		...
		// if we're combining files and minifying them
			...
			foreach ( $this->js [$group] as $ref ) :
				...
				if (! $ref ['combine']) :
					echo (($count_0 > 0) ? ',' : '') . (isset ( $ref ['prod'] )) ? $this->_tag_src ( 'js', $ref ['prod'] ) : $this->_tag_src ( 'js', $ref ['dev'] );
				 elseif (! $ref ['minify']) :
					$files [] = (isset ( $ref ['prod'] )) ? array ('prod' => $ref ['prod'], 'dev' => $ref ['dev'], 'minify' => $ref ['minify'] ) : array ('dev' => $ref ['dev'], 'minify' => $ref ['minify'] );
				 else :
					$files [] = (isset ( $ref ['prod'] )) ? array ('prod' => $ref ['prod'], 'dev' => $ref ['dev'] ) : array ('dev' => $ref ['dev'] );
			...
			echo $this->_tag_src ( 'js', $filename, TRUE );
		...
		// if we're minifying. but not combining
			...
			foreach ( $this->js [$group] as $ref ) :
				...
				echo (($count_0 > 0) ? ',' : '') . $this->_tag_src ( 'js', $f, TRUE );
		...
		// -- this will just display the production version if there is one, dev if there isn't.
			...
			foreach ( $this->js [$group] as $ref ) :
				...
				$f = (isset ( $ref ['prod'] )) ? $ref ['prod'] : $ref ['dev'];
				echo (($count_0 > 0) ? ',' : '') . $this->_tag_src ( 'js', $f );
	...
	/** 
	 * Display HTML SRC references to the css assets
	 * @access	private
	 * @param	String of the asset group name
	 * @return   Void
	 */
	private function _display_css_src($group = 'main') {
		...
		if ($this->dev) { // we're in a development environment
			...
			foreach ( $this->css [$group] as $media => $refs ) :
				...
				foreach ( $refs as $ref ) :
					...
					echo (($count_0 > 0 || $count_1 > 0) ? ',' : '') . $this->_tag_src ( 'css', $ref ['dev'], FALSE, $media );
			...
			foreach ( $this->css [$group] as $media => $refs ) :
				...
				foreach ( $refs as $ref ) :
					...
					if (! $ref ['combine']) :
						echo (($count_0 > 0 || $count_1 > 0) ? ',' : '') . (isset ( $ref ['prod'] )) ? $this->_tag_src ( 'css', $ref ['prod'], $media ) : $this->_tag_src ( 'css', $ref ['dev'], $media );
					 elseif (! $ref ['minify']) :
						$files [] = (isset ( $ref ['prod'] )) ? array ('prod' => $ref ['prod'], 'dev' => $ref ['dev'], 'minify' => $ref ['minify'] ) : array ('dev' => $ref ['dev'], 'minify' => $ref ['minify'] );
					 else :
						$files [] = (isset ( $ref ['prod'] )) ? array ('prod' => $ref ['prod'], 'dev' => $ref ['dev'] ) : array ('dev' => $ref ['dev'] );
				...
				echo (($count_0 > 0) ? ',' : '') . $this->_tag_src ( 'css', $filename, TRUE, $media );
			...
			foreach ( $this->css [$group] as $media => $refs ) :
				...
				foreach ( $refs as $ref ) :
					...
					echo (($count_0 > 0 || $count_1 > 0) ? ',' : '') . $this->_tag_src ( 'css', $f, TRUE, $media );
			...
			foreach ( $this->css [$group] as $media => $refs ) :
				...
				foreach ( $refs as $ref ) :
					...
					$f = (isset ( $ref ['prod'] )) ? $ref ['prod'] : $ref ['dev'];
					echo (($count_0 > 0 || $count_1 > 0) ? ',' : '') . $this->_tag_src ( 'css', $f, FALSE, $media );
	 ...
	 */
	private function _tag($flag, $ref, $cache = FALSE, $media = 'screen', $async = FALSE) {
		...
		switch ($flag) {
			...
			case 'js' :
				...
				if ($async == TRUE) {
					return '<script async src="' . $dir . $ref . '"></script>' . "\n\r";
	 ...
	 * @return	String containing an HTML tag reference to given reference
	 */
	private function _tag_src($flag, $ref, $cache = FALSE, $media = 'screen') {

...
class Secure extends Front_Controller {
	...
	function login($ajax = false) {
		...
		$this->data ['seo_title'] = lang ( 'login' );
		...
		//$this->data['ads']		= $this->banner_model->get_banners(true);
		$this->data ['categories'] = $this->Category_model->get_categories_tierd ( 0 );
	...
	function register() {
		...
		*/
		...
		$this->data ['page_title'] = lang ( 'account_registration' );
		$this->data ['seo_title'] = lang ( 'account_registration' );
		$this->data ['gift_cards_enabled'] = $this->gift_cards_enabled;
		...
		if ($this->form_validation->run () == FALSE) {
			...
			/*///////////////////////////////////////// CONFIRMATION EMAIL THRU CODEIGNITER EMAIL //////////////////////////////////////////
			...
			$row ['content'] = $this->load->view ( 'inscription_email_header', $this->data, true ) . $row ['content'] . $this->load->view ( 'order_email_footer', $this->data, true );
			...
			$this->mandrill = new Mandrill ( 'EtApXETYhBaLtQFfHI6sNw' );
			$message = array ('html' => $row ['content'], //'text' => 'Example text content',
			'subject' => $row ['subject'], 'from_email' => $this->config->item ( 'email' ), 'from_name' => $this->config->item ( 'company_name' ), 'to' => array (array ('email' => $save ['email'], 'name' => $save ['firstname'] . ' ' . $save ['lastname'], 'type' => 'to' ) ), 'headers' => array ('Reply-To' => $this->config->item ( 'email_contact' ) ), 'important' => true, 'track_opens' => true, 'track_clicks' => true, 'auto_text' => true, 'auto_html' => null, 'inline_css' => null, 'url_strip_qs' => null, 'preserve_recipients' => null, 'view_content_link' => null, 'bcc_address' => $this->config->item ( 'email_contact' ), 'tracking_domain' => 'clairetnathalie.com', 'signing_domain' => 'clairetnathalie.com', 'return_path_domain' => null, 'tags' => array ('inscription' ), 'subaccount' => null, 'google_analytics_domains' => array ('clairetnathalie.com' ), 'google_analytics_campaign' => 'message.from_email@clairetnathalie.com', 'metadata' => array ('website' => 'www.clairetnathalie.com' ) );
			$async = false;
			$ip_pool = 'Main Pool';
			$send_at = $sent_date_time;
			//$result = $this->CI->mandrill->messages->send($message, $async, $ip_pool, null);
			$result = $this->mandrill->messages->send ( $message, $async, $ip_pool, null );
			/*//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////*/
	...
	function forgot_password() {
		...
		$this->data ['seo_title'] = lang ( 'forgot_password' );
		$this->data ['gift_cards_enabled'] = $this->gift_cards_enabled;
		$submitted = $this->input->post ( 'submitted' );
		if ($submitted) {
	...
	function my_account($offset = 0) {
		...
		$this->data ['customer'] = ( array ) $this->Customer_model->get_customer ( $this->customer ['id'] );
		
		$this->data ['addresses'] = $this->Customer_model->get_address_list ( $this->customer ['id'] );
		
		$this->data ['page_title'] = 'Welcome ' . $this->data ['customer'] ['firstname'] . ' ' . $this->data ['customer'] ['lastname'];
		$this->data ['seo_title'] = lang ( 'account' );
		...
		$this->data ['orders_pagination'] = $this->pagination->create_links ();
		
		$this->data ['orders'] = $this->order_model->get_customer_orders ( $this->customer ['id'], $offset );
	...
	function my_downloads($code = false) {
		...
		if ($code !== false) {
			$this->data ['downloads'] = $this->Digital_Product_model->get_downloads_by_code ( $code );
		} else {
			$this->Customer_model->is_logged_in ();
			
			$customer = $this->go_cart->customer ();
			
			$this->data ['downloads'] = $this->Digital_Product_model->get_user_downloads ( $customer ['id'] );
		}
		
		$this->data ['gift_cards_enabled'] = $this->gift_cards_enabled;
		
		$this->data ['page_title'] = lang ( 'my_downloads' );
		$this->data ['seo_title'] = lang ( 'my_downloads' );
		
		$this->load->view ( 'my_downloads', $this->data );
	}
	...
	function checkout_address_manager() {
		...
		$this->data ['customer_addresses'] = $this->Customer_model->get_address_list ( $customer ['id'] );
		
		$this->load->view ( 'address_manager', $this->data );
	}
	...
	function address_manager_list_contents() {
		...
		$this->load->view ( 'address_manager_list_content', $this->data );
	}
	...
	function address_form($id = 0) {
		...
		if ($this->form_validation->run () == FALSE) {
			if (validation_errors () != '') {
				...
				$this->load->view ( 'address_form', $this->data );
			}
		} else {
			$a = array ();
			$a ['id'] = ($id == 0) ? '' : $id;

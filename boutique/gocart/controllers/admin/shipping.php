...
class Shipping extends Admin_Controller {
	...
	function settings($module) {
		...
		if (count ( $_POST ) > 0) {
			...
			$this->data ['form'] = $this->$module->form ( $this->session->flashdata ( 'post' ) );
		} else {
			$this->data ['form'] = $this->$module->form ();
		}
		$this->data ['module'] = $module;
		$this->data ['page_title'] = sprintf ( lang ( 'shipping_settings' ), $module );
		$this->load->view ( $this->config->item ( 'admin_folder' ) . '/shipping_module_settings', $this->data );
	}
}

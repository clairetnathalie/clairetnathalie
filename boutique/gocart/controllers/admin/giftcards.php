...
class Giftcards extends Admin_Controller {
	...
	function index() {
		...
		$this->data ['page_title'] = lang ( 'giftcards' );
		$this->data ['cards'] = $this->Gift_card_model->get_all_new ();
		
		$gc_settings = $this->Settings_model->get_settings ( 'gift_cards' );
		if (isset ( $gc_settings ['enabled'] )) {
			$this->data ['gift_cards'] ['enabled'] = $gc_settings ['enabled'];
		} else {
			$this->data ['gift_cards'] ['enabled'] = false;
		}
		
		$this->load->view ( $this->config->item ( 'admin_folder' ) . '/giftcards', $this->data );
	...
	function form() {
		...
		$this->data ['page_title'] = lang ( 'add_giftcard' );
		
		if ($this->form_validation->run () == FALSE) {
			$this->load->view ( $this->config->item ( 'admin_folder' ) . '/giftcard_form', $this->data );
		} else {
	...
	function settings() {
		...
		$this->data ['predefined_card_amounts'] = $gc_settings ['predefined_card_amounts'];
		$this->data ['allow_custom_amount'] = $gc_settings ['allow_custom_amount'];
		...
		$this->data ['page_title'] = lang ( 'giftcard_settings' );
		
		if ($this->form_validation->run () == FALSE) {
			$this->load->view ( $this->config->item ( 'admin_folder' ) . '/giftcards_settings', $this->data );
		} else {

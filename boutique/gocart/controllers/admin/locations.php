...
class Locations extends Admin_Controller {
	...
	function index() {
		$this->data ['page_title'] = lang ( 'countries' );
		$this->data ['locations'] = $this->Location_model->get_countries ();
		
		$this->load->view ( $this->config->item ( 'admin_folder' ) . '/countries', $this->data );
	}
	...
	function country_form($id = false) {
		...
		$this->data ['page_title'] = lang ( 'country_form' );
		...
		if ($this->form_validation->run () == FALSE) {
			$this->load->view ( $this->config->item ( 'admin_folder' ) . '/country_form', $this->data );
		} else {
			$save ['id'] = $id;
	...
	function zones($country_id) {
		$this->data ['countries'] = $this->Location_model->get_countries ();
		$this->data ['country'] = $this->Location_model->get_country ( $country_id );
		if (! $this->data ['country']) {
			$this->session->set_flashdata ( 'error', lang ( 'error_zone_not_found' ) );
			redirect ( $this->config->item ( 'admin_folder' ) . '/locations' );
		}
		$this->data ['zones'] = $this->Location_model->get_zones ( $country_id );
		
		$this->data ['page_title'] = sprintf ( lang ( 'country_zones' ), $this->data ['country']->name );
		
		$this->load->view ( $this->config->item ( 'admin_folder' ) . '/country_zones', $this->data );
	}
	...
	function zone_form($id = false) {
		...
		$this->data ['countries'] = $this->Location_model->get_countries ();
		$this->data ['page_title'] = lang ( 'zone_form' );
		...
		if ($id) {
			...
			$this->data = array_merge ( $this->data, $zone );
		}
		...
		if ($this->form_validation->run () == FALSE) {
			$this->load->view ( $this->config->item ( 'admin_folder' ) . '/country_zone_form', $this->data );
		} else {
			$save ['id'] = $id;
...
endforeach
	...
	function zone_areas($id) {
		$this->data ['zone'] = $this->Location_model->get_zone ( $id );
		$this->data ['areas'] = $this->Location_model->get_zone_areas ( $id );
		
		$this->data ['page_title'] = sprintf ( lang ( 'zone_areas_for' ), $this->data ['zone']->name );
		
		$this->load->view ( $this->config->item ( 'admin_folder' ) . '/country_zone_areas', $this->data );
	}
	...
	function zone_area_form($zone_id, $area_id = false) {
		...
		$this->data ['page_title'] = sprintf ( lang ( 'zone_area_form' ), $zone->name );
		...
		if ($area_id) {
			...
			$this->data = array_merge ( $this->data, $area );
		}
		...
		if ($this->form_validation->run () == FALSE) {
			$this->load->view ( $this->config->item ( 'admin_folder' ) . '/country_zone_area_form', $this->data );
		} else {
			$save ['id'] = $area_id;

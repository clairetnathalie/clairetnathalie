...
class Boxes extends Admin_Controller {
	...
	function index() {
		$this->data ['boxes'] = $this->Box_model->get_boxes ();
		$this->data ['page_title'] = lang ( 'boxes' );
		
		$this->load->view ( $this->config->item ( 'admin_folder' ) . '/boxes', $this->data );
	}
	...
	function form($id = false) {
		...
		$this->data = array ('id' => $id, 'title' => '', 'enable_on' => '', 'disable_on' => '', 'image' => '', 'link' => '', 'new_window' => false, 'show' => true );
		if ($id) {
			$this->data = ( array ) $this->Box_model->get_box ( $id );
			...
			$this->data ['show'] = ( bool ) $this->data ['show'];
		}
		$this->data ['page_title'] = lang ( 'box_form' );
		...
		$this->form_validation->set_rules ( 'show', 'lang:show', 'trim' );
		
		if ($this->form_validation->run () == false) {
			$this->data ['error'] = validation_errors ();
			$this->load->view ( $this->config->item ( 'admin_folder' ) . '/box_form', $this->data );
		} else {
			...
			$save ['show'] = $this->input->post ( 'show' );

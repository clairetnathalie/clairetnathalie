...
<fieldset><label for="name"><?php echo lang('name');?></label>
				<?php
...
<fieldset><label for="slug"><?php echo lang('slug');?> </label>
				<?php
...
<div class="input-append">
	...
	class="add-on"><?php echo lang('max_file_size');?> <?php echo  $this->config->item('size_limit')/1024; ?>kb</span>
</div>
					
				<?php if($id && $image != ''):?>
				
				<div
	style="text-align: center; padding: 5px; border: 1px solid #ddd;"><img
	src="<?php echo base_url(((!preg_match('/localhost/', current_url()))?'':'clairetnathalie/').IMG_UPLOAD_FOLDER.'images/small/'.$image);?>"
...
<fieldset><label for="seo_title"><?php echo lang('seo_title');?> </label>
				<?php

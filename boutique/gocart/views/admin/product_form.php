...
function add_product_image(data)
{
	...
	var photo = '<?php add_image("'+p[0]+'", "'+p[0]+'.'+p[1]+'", '', '', '', base_url(((!preg_match('/localhost/', current_url()))?'':'clairetnathalie/').IMG_UPLOAD_FOLDER.'images/thumbnails'));?>';
...
<div class="row">
<div class="span8"><label><?php echo lang('excerpt');?></label>
						<?php
...
<div class="row">
<div class="span8">
<fieldset><legend><?php echo lang('inventory');?></legend>
<div class="row" style="padding-top: 10px;">
<div class="span3"><label for="track_stock"><?php echo lang('track_stock');?> </label>
									<?php
									$options = array ('1' => lang ( 'yes' ), '0' => lang ( 'no' ) );
									echo form_dropdown ( 'track_stock', $options, set_value ( 'track_stock', $track_stock ), 'class="span3"' );
									?>
								</div>
<div class="span3"><label for="fixed_quantity"><?php echo lang('fixed_quantity');?> </label>
									<?php
...
<div class="span2"><label for="quantity"><?php echo lang('quantity');?> </label>
									<?php
...
<div class="row">
<div class="span8">
<fieldset><legend><?php echo lang('header_information');?></legend>
<div class="row" style="padding-top: 10px;">
<div class="span8"><label for="slug"><?php echo lang('slug');?> </label>
									<?php
...
<div class="row">
<div class="span8"><label><strong><?php echo lang('select_a_category');?></strong></label>
</div>
</div>
<div class="row">
<div class="span2" style="text-align: center">
<div class="row">
<div class="span2"><input class="span2" type="text" id="category_search" />
<script type="text/javascript">
...
<div class="row">
<div class="span2"><select class="span2" id="category_list" size="5"
	style="margin: 0px;"></select></div>
</div>
<div class="row">
<div class="span2" style="margin-top: 8px;"><a href="#"
	onclick="add_category(); return false;" class="btn"
	title="Add Category"><?php echo lang('add_category');?></a></div>
</div>
</div>
<div class="span6">
...
<div class="row">
<div class="span8">
<div class="pull-right" style="padding: 0px 0px 10px 0px;"><select
	id="option_options" style="margin: 0px;">
	...
	value="<?php echo lang('add_option');?>" style="margin: 0px;" /></div>
</div>
</div>
...
<div class="row">
<div class="span8"><label><strong><?php echo lang('select_a_product');?></strong></label>
</div>
</div>
<div class="row">
<div class="span2" style="text-align: center">
<div class="row">
<div class="span2"><input class="span2" type="text" id="product_search" />
<script type="text/javascript">
...
<div class="row">
<div class="span2"><select class="span2" id="product_list" size="5"
	style="margin: 0px;"></select></div>
</div>
<div class="row">
<div class="span2" style="margin-top: 8px;"><a href="#"
	onclick="add_related_product();return false;" class="btn"
...
<div class="row"><iframe id="iframe_uploader"
	src="<?php echo site_url($this->config->item('admin_folder').'/products/product_image_form');?>"
	class="span8" style="height: 75px; border: 0px;"></iframe></div>
<div class="row">
<div class="span8">
...
<div class="span4">
		...
		<?php
		$options = array ('1' => lang ( 'shippable' ), '0' => lang ( 'not_shippable' ) );
		echo form_dropdown ( 'shippable', $options, set_value ( 'shippable', $shippable ), 'class="span4"' );
		?>
		
		<?php
		$options = array ('1' => lang ( 'taxable' ), '0' => lang ( 'not_taxable' ) );
		...
		<label for="weight"><?php echo lang('weight');?> (kg)</label>
		<?php
		...
		<?php preg_match('/(Housse "|Duvet Cover ")/s', $name, $match_productName);?>
		
		<?php
		//if($match_productName)
		//if($match_productName || $name == '')
		if ($match_productName && $enabled == 1 && $quantity > 0) {
			if ($show_connected_product_bool == false) {
				$is_visible = false;
			} else {
				$is_visible = true;
			}
		...
		<div
	class="form-actions <?php echo ($is_visible == true)?'':'hide';?>"
	id="form-connected-product">
<div class="control-group info"><label><?php echo lang('connected_product');?></label>
<br>
<div class="controls form-horizontal">
				<?php
				$options = array ('1' => lang ( 'connected' ), '0' => lang ( 'not_connected' ) );
				echo form_dropdown ( 'connected_product_bool', $options, set_value ( 'connected_product_bool', $connected_product_bool ), 'id="connect_product_switch" class="span4" style="max-width: 330px;"' );
				?>
				
				</div>
</div>
<br>
<div class="tab-pane" id="connected_product_options">
<div class="row">
<div class="span4"><label><strong><?php echo lang('select_a_connected_product');?></strong></label>
</div>
</div>
<div class="row">
<div class="span4" style="text-align: center">
<div class="row">
<div class="span3"><input class="span3" style="max-width: 330px;"
	type="text" id="connected_product_search"
	placeholder="<?php echo lang('search_by_name_of_product');?>" /> <script
	type="text/javascript">
								$('#connected_product_search').keyup(function(){
									$('#connected_product_list').html('');
									run_connected_product_query();
								});

								function run_connected_product_query()
								{
									$.post("<?php echo site_url($this->config->item('admin_folder').'/products/product_autocomplete/');?>", { name: $('#connected_product_search').val(), limit:10},
										function(data) {
											...
											$.each(data, function(index, value){
												...
												if($('#connected_product_'+index).length == 0)
												{
													$('#connected_product_list').append('<option id="connected_product_item_'+index+'" value="'+index+'">'+value+'</option>');
												}
											});
...
<div class="row">
<div class="span3" style=""><select class="span4"
	id="connected_product_list" size="5"
	style="margin: 0px; max-width: 330px;"></select></div>
</div>
<div class="row">
<div class="span3" style="margin-top: 12px; max-width: 330px;"><a
	href="#" onclick="add_connected_product(); return false;" class="btn"
	title="Add Connected Product"><?php echo lang('add_connected_product');?></a>
</div>
</div>
</div>
<div class="span4" style="max-width: 330px;">
<table class="table table-striped" style="margin-top: 10px;">
	<tbody id="connected_product_items_container">
							<?php
							foreach ( $connected_parents as $con ) {
								echo connected_items ( $con->id, $con->name );
							}
							?>
							</tbody>
</table>
...
<div class="span2"><input type="hidden"
	name="images[<?php echo $photo_id;?>][filename]"
	value="<?php echo $filename;?>" /> <img class="gc_thumbnail"
	src="<?php echo base_url(((!preg_match('/localhost/', current_url()))?'':'clairetnathalie/').IMG_UPLOAD_FOLDER.'images/thumbnails/'.$filename);?>"
	style="padding: 5px; border: 1px solid #ddd" /></div>
<div class="span6">
<div class="row">
<div class="span2"><input name="images[<?php echo $photo_id;?>][alt]"
	value="<?php echo $alt;?>" class="span2"
	placeholder="<?php echo lang('alt_tag');?>" /></div>
<div class="span2"><input type="radio" name="primary_image"
	value="<?php echo $photo_id;?>"
	<?php if($primary) echo 'checked="checked"';?> /> <?php echo lang('primary');?>
				</div>
<div class="span2"><a onclick="return remove_image($(this));"
	rel="<?php echo $photo_id;?>" class="btn btn-danger"
	style="float: right; font-size: 9px;"><i class="icon-trash icon-white"></i> <?php echo lang('remove');?></a>
</div>
</div>
<div class="row">
<div class="span6"><label><?php echo lang('caption');?></label> <textarea
	name="images[<?php echo $photo_id;?>][caption]" class="span6" rows="3"><?php echo $caption;?></textarea>
</div>
</div>
</div>
</div>
...
<tr id="option-<?php echo $count;?>">
	...
	<button type="button" class="btn btn-mini btn-danger pull-right"
		onclick="remove_option(<?php echo $count ?>);"><i
		class="icon-trash icon-white"></i></button>
	<input type="hidden" name="option[<?php echo $count;?>][type]"
		value="<?php echo $po->type;?>" />
	<div class="option-form" id="option-form-<?php echo $count;?>">
	<div class="row-fluid">

	<div class="span10"><input type="text" class="span10"
		placeholder="<?php echo lang('option_name');?>"
		name="option[<?php echo $count;?>][name]"
		value="<?php echo $po->name;?>" /></div>

	<div class="span2" style="text-align: right;"><input class="checkbox"
		type="checkbox" name="option[<?php echo $count;?>][required]"
		value="1" <?php echo ($po->required)?'checked="checked"':'';?> /> <?php echo lang('required');?>
					</div>
	</div>
				<?php if($po->type!='textarea' && $po->type!='textfield'):?>
				<div class="row-fluid">
	<div class="span12"><a class="btn"
	...
	<div class="span2"><strong>&nbsp;<?php echo lang('weight');?> (kg)</strong></div>
...
<div class="row-fluid">
			<?php if($po->type!='textarea' && $po->type!='textfield'):?><div
	class="span1"><a class="handle btn btn-mini" style="float: left;"><i
...
{
	//if the category is not already an associated category, add it
...
function add_connected_product()
{
	//if the connected product is not already a connected product, add it
	if($('#connected_product_'+$('#connected_product_list').val()).length == 0 && $('#connected_product_list').val() != null)
	{
		<?php $new_item	 = str_replace(array("\n", "\t", "\r"),'',connected_items("'+$('#connected_product_list').val()+'", "'+$('#connected_product_item_'+$('#connected_product_list').val()).html()+'"));?>
		var connected_product = '<?php echo $new_item;?>';
		$('#connected_product_items_container').append(connected_product);
		run_connected_product_query();
	}
	else
	{
		if($('#connected_product_list').val() == null)
		{
			alert('<?php echo lang('alert_select_product_connected');?>');
		}
		else
		{
			alert('<?php echo lang('alert_product_connected');?>');
		}
	}
}
...
function remove_connected_product(id)
{
	if(confirm('<?php echo lang('confirm_remove_connected');?>'))
	{
		$('#connected_product_'+id).remove();
		run_connected_product_query();

		if($('#connected_product_items_container').children().length == 0)
		{
			$('select[name="connected_product_bool"]').val( 0 ).prop('selected',true);
			$('#connected_product_options').addClass('hide');
		}
	}
}
...
<?php if($this->session->userdata('db_active_group') && $this->session->userdata('db_active_group') == 'french') : ?>
var pattern = /^Housse[\s]{1}"(.)+"/;
<?php endif;?>
<?php if($this->session->userdata('db_active_group') && $this->session->userdata('db_active_group') == 'english') : ?>
var pattern = /^Duvet[\s]{1}Cover[\s]{1}"(.)+"/;
<?php endif;?>

$('input[name="name"]').bind("keyup change", function(e) {
	var string = $(this).val();
	if( $('input[name="quantity"]').val() != 0 && $('select[name="enabled"] option:selected').val() == 1)
	{
		if(string.match(pattern))
		{
			$('#form-connected-product').removeClass('hide');
		}
		else
		{
			$('select[name="connected_product_bool"]').val( 0 ).prop('selected',true);
			$('#connected_product_options').addClass('hide');
			$('#form-connected-product').addClass('hide');
		}
	}
	else
	{
...
$('input[name="quantity"]').bind("keyup change", function(e) {
	var string = $('input[name="name"]').val();
	if( $('input[name="quantity"]').val() != 0 && $('select[name="enabled"] option:selected').val() == 1)
	{
		if(string.match(pattern))
...
$(document).ready(function() {
	...
	if($('select[name="connected_product_bool"] option:selected').val() == 0)
	{
		$('#connected_product_options').addClass('hide');
	}
	else
...
$('select[name="connected_product_bool"]').change(function() {
	$('select[name="connected_product_bool"] option:selected').each(function () {
	if($(this).val() == 0)
	{
		$('#connected_product_options').addClass('hide');
	}
	...
	{
		$('#connected_product_options').removeClass('hide');
	}	
	});
});
...
function connected_items($id, $name) {
	return '
			<tr id="connected_product_' . $id . '">
				<td>
					<input type="hidden" name="connected_parent_product_ids[]" value="' . $id . '"/>
					' . $name . '</td>
				<td>
					<a class="btn btn-danger pull-right btn-mini" href="#" onclick="remove_connected_product(' . $id . '); return false;"><i class="icon-trash icon-white"></i> ' . lang ( 'remove' ) . '</a>
				</td>
			</tr>
		';
}

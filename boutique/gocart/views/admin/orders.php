...
<div class="span8">
				<?php echo form_open($this->config->item('admin_folder').'/orders/index', 'class="form-inline" style="float:right"');?>
					<fieldset><input id="start_top" value="" class="span2" type="text"
	placeholder="Start Date" /> <input id="start_top_alt" type="hidden"
	name="start_date" /> <input id="end_top" value="" class="span2"
	type="text" placeholder="End Date" /> <input id="end_top_alt"
	type="hidden" name="end_date" /> <input id="top" type="text"
	class="span2" name="term" placeholder="<?php echo lang('term')?>" />
...
<table class="table table-striped">
	<thead>
		<tr>
			...
			<th style="text-align: right; padding-right: 20px;"><?php echo sort_url('total','total', $sort_by, $sort_order, $code, $this->config->item('admin_folder')); ?></th>
			<th></th>
		</tr>
	</thead>
	...
	<tr>
			...
			<td style="padding-top: 12px;"><?php echo $order->order_number; ?></td>
			<td style="white-space: nowrap; padding-top: 12px;"><?php echo $order->bill_lastname.', '.$order->bill_firstname; ?></td>
			<td style="white-space: nowrap; padding-top: 12px;"><?php echo $order->ship_lastname.', '.$order->ship_firstname; ?></td>
			<td style="white-space: nowrap; padding-top: 12px;"><?php echo date('m/d/y h:i a', strtotime($order->ordered_on)); ?></td>
			<td class="span5">
			<?php
					$select_colors_array = ( array ) $this->config->item ( 'order_statuses_colors' );
					$select_class = 'class="span5 selectpicker" data-style="btn-' . $select_colors_array [$order->status] . '" style="max-width: 150px;"';
					?>
			<?php echo form_dropdown('status', $this->config->item('order_statuses'), $order->status, 'id="status_form_'.$order->id.'" '.$select_class.' style="float:left;"'); ?>
			<br />
			<button type="button" class="btn"
				onClick="save_status(<?php echo $order->id; ?>)"
				style="float: left; margin-left: 0;"><?php echo lang('save');?></button>
			</td>
			<td class="span2"
				style="padding-top: 12px; min-width: 40px; text-align: right; padding-right: 20px;">
			<div class="MainTableNotes"><?php echo number_format(abs( (float)$order->total ), 2, $this->config->item('currency_decimal'), $this->config->item('currency_thousands_separator') ) . ' €'; if($order->currency != 'EUR') { echo '<br><small>' . number_format(abs( ((float)$order->total)*((float)$order->currency_rate) ), $order->currency_decimal_place, $this->config->item('currency_decimal'), $this->config->item('currency_thousands_separator')) . ' ' . $order->currency . '</small>'; }; ?></div>
			</td>
			<td><a class="btn btn-small"
				style="float: right; padding-top: 5px; min-height: 20px;"
				href="<?php echo site_url($this->config->item('admin_folder').'/orders/view/'.$order->id);?>"><i
				class="icon-search"></i> <?php echo lang('form_view')?></a></td>
		</tr>
    <?php endforeach; ?>
    </tbody>
</table>
...
function submit_form()
{
	...
	{
		<?php
		$dataAlert = array ('myString' => lang ( 'error_no_orders_selected' ) );
		?>
		var escapeAlert = <?php echo json_encode($dataAlert) ?>;
		alert(escapeAlert.myString);
		return false;
	}
}

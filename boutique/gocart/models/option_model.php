...
class Option_model extends CI_Model {
	...
	function validate_product_options(&$product, $post_options) {
		...
		if ($this->config->item ( 'language' ) == 'french') {
			$msg = 'Les options suivantes n\'ont pas été sélectionnées et sont requises:<br/>';
		} else {
		...
		foreach ( $db_options as $option ) {
			...
			// handle text fields
				...
				if ($option_value) {
					if ($option->name == 'Housse' || $option->name == 'Duvet Cover') {
		...
		if ($error) {
			return (array ('validated' => false, 'message' => $msg ));
		} else {

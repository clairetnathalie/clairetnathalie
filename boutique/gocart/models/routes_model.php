...
class Routes_model extends CI_Model {
	...
	/*
		if($db_active_group == 'french')
		{
			$this->file_name = APPPATH.'config/routes_fr'.EXT;	
		}
		else
		{
			$this->file_name = APPPATH.'config/routes_en'.EXT;
		}
		*/
	}
	...
	function exists($route) {
		if (array_key_exists ( $route, $this->router->routes ))
			return true;
	}
	...
	function update($old_slug, $new_slug, $path) {
		...
		if (! $this->exists ( $old_slug ))
			return $this->append ( $new_slug, $path );
		
		// Is the new valid ?
		if (! $this->verify ( $new_slug ))
			return false;

...
class Digital_Product_Model extends CI_Model {
	...
	function new_file() {
		return array ('id' => '', 'filename' => '', 'max_downloads' => '', 'title' => '', 'description' => '', 'size' => '' );
	}
	...
	function get_downloads_by_code($code) {
		...
		if ($row) {
			return array ($row->order_id => $this->get_package_files ( $row->id ) );
		}
	}
	...
	function get_package_files($package_id) {
		...
		return $this->db->select ( '*' )->from ( 'download_package_files as a' )->join ( 'digital_products as b', 'a.file_id=b.id' )->where ( 'package_id', $package_id )->get ()->result ();
	}
	...
	function get_file_info_by_link($link) {
		...
		$record = $this->db->from ( 'digital_products as a' )->join ( 'download_package_files as b', 'a.id=b.file_id' )->where ( 'link', $link )->get ()->row ();

				...
				if ($group) // group might not exist
{
	...
	function reset_password($email) {
		...
		if ($customer) {
			...
			$this->email->to ( $email );
			$this->email->subject ( $this->config->item ( 'site_name' ) . ': ' . lang ( 'reset_password_message_subject' ) );
			$this->email->message ( sprintf ( lang ( 'reset_password_message_body' ), $new_password ) );
			$this->email->send ();
	...
	function get_customer_id_by_email($email) {
		$id = $this->db->get_where ( 'customers', array ('email' => $email ) )->row ( 'id' );
		return $id;
	}

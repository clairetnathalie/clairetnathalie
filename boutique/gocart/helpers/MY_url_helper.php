...
function theme_url($uri) {
	...
	if (file_exists ( $CI->config->config ['asset_url'] . $CI->config->item ( 'theme' ) . '/' . $uri )) {
		$file_time = "?" . filemtime ( $CI->config->config ['asset_url'] . $CI->config->item ( 'theme' ) . '/' . $uri );
	} else {
		$file_time = "";
	}
	
	return $CI->config->config ['asset_url'] . $CI->config->item ( 'theme' ) . '/' . $uri . $file_time;
}
...
function theme_js($uri, $tag = false, $async = false) {
	if ($tag) {
		if ($async) {
			return '<script async src="' . theme_url ( 'assets/js/' . $uri ) . '"></script>';
		} else {

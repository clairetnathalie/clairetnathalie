...
function format_currency($value, $symbol = true) {
	...
	if ($symbol) {
		$formatted = number_format ( abs ( (( float ) $value) * (( float ) $CI->session->userdata ( 'currency_rate' )) ), $CI->session->userdata ( 'currency_decimal_place' ), $CI->config->item ( 'currency_decimal' ), $CI->config->item ( 'currency_thousands_separator' ) );
		
		if ($CI->session->userdata ( 'currency_symbol_side' ) == 'left') {
			$formatted = $neg . $CI->session->userdata ( 'currency_symbol' ) . $formatted;
		} else {
			$formatted = $neg . $formatted . $CI->session->userdata ( 'currency_symbol' );
		}
	} else {
		//traditional number formatting
		$formatted = number_format ( abs ( (( float ) $value) * (( float ) $CI->session->userdata ( 'currency_rate' )) ), $CI->session->userdata ( 'currency_decimal_place' ), '.', ',' );
	}

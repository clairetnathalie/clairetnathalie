...
class Paypal_express {
	...
	function __construct() {
		...
		$this->CI->load->library ( 'httprequest_pp' );
	...
	*/
	...
	/*
		//FOR TESTING PAYPAL SERVER RESPONSE
		return $this->CI->paypal->doExpressCheckout($this->CI->go_cart->total(), $store.' order');
		*/
	}
	
	public function complete_payment($data) {
		$order_id = $data ['order_id'];
		$paypal_transaction_id = $this->CI->go_cart->get_paypal_transaction_id ();
		$quid_2 = $this->CI->go_cart->get_quid_2 ();
		
		$this->CI->Order_model_mgm->confirm_order_payment_paypal_express ( $order_id, $paypal_transaction_id, $quid_2 );
	}

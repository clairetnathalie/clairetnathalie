...
table_rate is now the standard table rate shipping module and if you are
using this method you should migrate to the new one.</h3>
<style type="text/css">
.tablerate_input {
	width: 50px;
}
...
foreach ( $rates as $name => $ratelist ) {
...
<div id='add_name'> <?php echo lang('add_tbl') ?> <input type="text"
	id="add_name_input" class="gc_tf1" /> <input type="button" value="Add"
	onclick='add_table()' /></div>
<div id="tabs">

<ul id='name_list'>
		<?php foreach($rates as $name=>$ratelist ) {  ?>
			 <li><a href="#tbl_<?php echo $name ?>"
		id='label_<?php echo $name ?>'><?php echo str_replace('_', ' ', $name); ?></a></li>
		<?php } ?>
		</ul>
		<?php
		
foreach ( $rates as $name => $table ) {
			$disp_name = str_replace ( '_', ' ', $name );
			?>
		<div id='tbl_<?php echo $name ?>'><strong><span
	id='<?php echo $name ?>_name'><?php echo $disp_name ?></span> <?php echo lang('rates') ?></strong>
(<a href="javascript:void(0)"
	onclick="remove_table('tbl_<?php echo $name ?>')"><?php echo lang('btn_tbl_remove') ?></a>)
(<a href="javascript:void(0)"
	onclick="rename_table('<?php echo $name ?>')"><?php echo lang('btn_rename') ?></a>)
<table>
	<tr>

		<div id="rename_<?php echo $name ?>" style="display: none"><input
			type="text" id="input_<?php echo $name ?>"
			value="<?php echo $disp_name ?>" class="gc_tf1"> <input type="button"
			value="Change"
			onclick="apply_rename('<?php echo $name ?>', '<?php echo $name ?>')" />
		<input type="button" value="Cancel"
			onclick="cancel_rename('<?php echo $name ?>')" /></div>
	...
	<tr>
		...
		<td style="font-size: 11px;"><a
			href="javascript:tablerate_remove('<?php echo $name ?>_<?php echo $count ?>');">(<?php echo lang('btn_remove') ?>)</a>
		<a
			href="javascript:tablerate_add_above('<?php echo $name ?>',<?php echo $count ?>)">(<?php echo lang('btn_above') ?>)</a>
		<a
			href="javascript:tablerate_add_below('<?php echo $name ?>',<?php echo $count ?>)">(<?php echo lang('btn_below') ?>)</a>
		</td>
	</tr>
			<?php
				$count ++;

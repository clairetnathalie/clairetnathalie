...
class coursier_banlieu_parisienne {
	var $CI;
	var $cart;
	
	function coursier_banlieu_parisienne() {
		$this->CI = & get_instance ();
		$this->CI->lang->load ( 'coursier_banlieu_parisienne' );
	}
	...
	function rates() {
		...
		if (! preg_match ( '/(77\d{3})|(78\d{3})|(91\d{3})|(92\d{3})|(93\d{3})|(94\d{3})|(95\d{3})/', $customer ['ship_address'] ['zip'] )) {
			return array ();
		}
		
		$settings = $this->CI->Settings_model->get_settings ( 'coursier_banlieu_parisienne' );
	...
	function install() {
		...
		$this->CI->Settings_model->save_settings ( 'coursier_banlieu_parisienne', $install );
	}
	
	function uninstall() {
		$this->CI->Settings_model->delete_settings ( 'coursier_banlieu_parisienne' );
	}
	...
	function form() {
		...
		return $this->CI->load->view ( 'coursier_banlieu_parisienne_form', $data, true );
	...
	function check() {
		...
		$this->CI->Settings_model->save_settings ( 'coursier_banlieu_parisienne', $save );

...
class fedex {
	...
	function rates() {
		...
		$request ['RequestedShipment'] ['Shipper'] = array ('Address' => array ('StreetLines' => array ($this->CI->config->item ( 'address1' ), $this->CI->config->item ( 'address2' ) ), // Origin details
'City' => $this->CI->config->item ( 'city' ), 'StateOrProvinceCode' => $this->CI->config->item ( 'state' ), 'PostalCode' => $this->CI->config->item ( 'zip' ), 'CountryCode' => $this->CI->config->item ( 'country' ) ) );
		$request ['RequestedShipment'] ['Recipient'] = array ('Address' => array ('StreetLines' => array ($customer_address ['address1'], $customer_address ['address2'] ), // Destination details
'City' => $customer_address ['city'], 'StateOrProvinceCode' => $customer_address ['state'], 'PostalCode' => $customer_address ['zip'], 'CountryCode' => $customer_address ['country_code'], 'Residential' => true ) );
		...
		$request ['RequestedShipment'] ['RequestedPackageLineItems'] = array ('0' => array ('Weight' => array ('Value' => $weight, 'Units' => $this->config->item ( 'weight_unit' ) ), 'Dimensions' => array ('Length' => $pkg_length, 'Width' => $pkg_width, 'Height' => $pkg_height, 'Units' => $this->config->item ( 'dimension_unit' ) ) ) );
					...
					if (is_numeric ( $handling_amount )) // valid entry?
{
	...
	function install() {
		$default_settings = array ('enabled' => 0, 'key' => '', 'password' => '', 'meter' => '', 'shipaccount' => '', 'handling_method' => '$', 'handling_amount' => 5, 'length' => 10, 'width' => 10, 'height' => 3, 'package' => 'YOUR_PACKAGING', 'service' => implode ( ',', array_keys ( $this->service_list ) ) );
	...
	function form() {
		...
		if (! $_POST) {
			...
			$meter = $_POST ['meter'];
			$handling_method = $_POST ['handling_method'];
...
<label class="checkbox"> <input type="checkbox" name="service[]"
	value="<?php echo $id;?>"
	<?php echo (in_array($id, $service))?'checked="checked"':'';?> />
		<?php echo $opt;?>
	</label>
<?php endforeach;?>
